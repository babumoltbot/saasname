...
export default function UpgradePrompt({ onClose }: Props) {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setPaymentError(data.error || "Something went wrong. Please try again later.");
        setLoading(false);
        return;
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay backdrop-blur-md">
      <div className="animate-scale-in relative bg-surface border border-border rounded-2xl p-8 max-w-sm mx-4 text-center overflow-hidden">
        ...
        <div className="relative">
          {stripeDown ? (
            <>
              <div className="w-14 h-14 mx-auto mb-5 flex items-center justify-center bg-amber-500/10 border border-amber-500/20 rounded-2xl">
              ...
              <p className="text-text-secondary text-sm leading-relaxed mb-3">
                Thanks for wanting to try PikName! We&apos;re sorting out a temporary issue with our payment provider.
              </p>
              <p className="text-text-secondary text-sm leading-relaxed mb-8">
                As a thank you for your patience, we&apos;ll set you up with a complimentary early access plan so you can start generating names right away. You&apos;ll receive an email once it&apos;s activated — shouldn&apos;t be long.
              </p>

              <button
                onClick={onClose}
                className="w-full py-3.5 text-sm font-semibold text-black bg-accent rounded-xl hover:translate-y-[-1px] hover:shadow-[0_0_30px_var(--color-accent-glow)] transition-all"
              >
                Got it, thanks
              </button>
            </>
            ...
            <>
              ...
              {paymentError && (
                <div className="mb-4 p-3 bg-red-500/[0.06] border border-red-500/20 rounded-xl">
